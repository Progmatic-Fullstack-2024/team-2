...
export default function LoginForm() {
  ...
  const handleLogin = async (values) => {
    ...
    if (result.ok) {
      ...
      navigate('/signedIn'); // Ez az url a bejelntkezett felhasználós oldalra visz!
    } else {
      const errorMessage = result.message?.respone?.data?.error || 'Ismeretlen hiba';
      const statusCode = result.message?.respone?.status || 'N/A';
