      ...
      <div className={dropdownMenuClass + (dropdownMenuOpen ? ' block' : ' hidden')}>
        <ul className="text-sm text-gray-700 self-end">
          {Object.keys(menuItems).map((value) => {
            if (value != currentItem.current)
              return (
                <li
                  key={value}
                  className="mt-0.5 block px-4 py-1 select-none hover:text-c-primary-dark hover:ring-1 hover:ring-c-primary cursor-pointer border border-gray-200"
