...
const createPerformance = async (req, res, next) => {
	...
	try {
		...
		return next(
			new HttpError(error.message || "Failed to create performance", error.statusCode || 500)
		);
	}
};
...
const updatePerformance = async (req, res, next) => {
	const { performanceId } = req.params;
	const { title, theaterId, description, price, performanceDate, creatorsId } = req.body;
	...
	try {
		parsedCreatorsIds = creatorsId ? JSON.parse(creatorsId) : { toAdd: [], toRemove: [] };
	} catch (error) {
	...
	try {
		...
		return next(
			new HttpError(error.message || "Failed to update performance", error.statusCode || 500)
		);
	}
};
...
const destroyPerformance = async (req, res, next) => {
	...
	try {
		...
		return next(
			new HttpError(error.message || "Failed to delete performance", error.statusCode || 500)
		);
	}
};
...
const deleteImage = async (req, res, next) => {
	...
	try {
		...
		return next(new HttpError(error.message || "Failed to delete image", error.statusCode || 500));
	}
};
