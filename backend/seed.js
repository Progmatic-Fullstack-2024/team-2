...
async function main() {
	...
	const performanceTitles = [
		"Csillagok Tánca",
		"Éjféli Harmónia",
		"Naplemente Szimfónia",
		"Holdas Álmok",
		"Tavaszi Reggel",
		"Kísérleti Szenvedély",
		"Komédiák Kertje",
		"Tragédia a Fák Alatt",
		"Monodráma az Éjben",
		"Zenés Varázslat",
	];
	...
	for (const [i, theater] of theaters.entries()) {
		for (let j = 0; j < 5; j++) {
			const randomGenre = genreList[Math.floor(Math.random() * genreList.length)]; // Pick a random genre from the list

			const performanceTitle = performanceTitles[(i * 5 + j) % performanceTitles.length];
